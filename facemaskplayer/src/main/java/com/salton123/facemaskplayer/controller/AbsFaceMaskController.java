...
import com.salton123.facemaskplayer.IFaceMaskPlayer;
import com.salton123.facemaskplayer.Mode;
import com.salton123.facemaskplayer.State;
 ...
 * Description:
 */
public abstract class AbsFaceMaskController extends FrameLayout implements View.OnTouchListener {
    public AbsFaceMaskController(@NonNull Context context) {
        super(context);
    }

    public AbsFaceMaskController(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public AbsFaceMaskController(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    protected IFaceMaskPlayer mNiceVideoPlayer;
    ...
    public void setPlayer(IFaceMaskPlayer niceVideoPlayer) {
     ...
     *                  <ul>
     *                  <li>{@link State#STATE_IDLE}</li>
     *                  <li>{@link State#STATE_PREPARING}</li>
     *                  <li>{@link State#STATE_PREPARED}</li>
     *                  <li>{@link State#STATE_PLAYING}</li>
     *                  <li>{@link State#STATE_PAUSED}</li>
     *                  <li>{@link State#STATE_BUFFERING_PLAYING}</li>
     *                  <li>{@link State#STATE_BUFFERING_PAUSED}</li>
     *                  <li>{@link State#STATE_ERROR}</li>
     *                  <li>{@link State#STATE_COMPLETED}</li>
     *                  </ul>
     */
     ...
     *                 <ul>
     *                 <li>{@link Mode#MODE_NORMAL}</li>
     *                 <li>{@link Mode#MODE_FULL_SCREEN}</li>
     *                 <li>{@link Mode#MODE_TINY_WINDOW}</li>
     *                 </ul>
     */
    ...
    public void startUpdateProgressTimer() {
        ...
        if (mUpdateProgressTimerTask == null) {
            mUpdateProgressTimerTask = new TimerTask() {
                @Override
                public void run() {
                    AbsFaceMaskController.this.post(new Runnable() {
                        @Override
                        public void run() {
                            updateProgress();
                        }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!mNeedChangePosition && !mNeedChangeVolume && !mNeedChangeBrightness) {
                    ...
                    if (absDeltaX >= THRESHOLD) {
                        ...
                        mGestureDownPosition = mNiceVideoPlayer.current();
                    } else if (absDeltaY >= THRESHOLD) {
                        if (mDownX < getWidth() * 0.5f) {
                            // 左侧改变亮度
                            mNeedChangeBrightness = true;
                            mGestureDownBrightness = NiceUtil.scanForActivity(getContext())
                ...
                if (mNeedChangePosition) {
                    long duration = mNiceVideoPlayer.duration();
                ...
                if (mNeedChangeBrightness) {
                    ...
                    WindowManager.LayoutParams params = NiceUtil.scanForActivity(getContext())
                            .getWindow().getAttributes();
                    params.screenBrightness = newBrightnessPercentage;
                    NiceUtil.scanForActivity(getContext()).getWindow().setAttributes(params);
                    int newBrightnessProgress = (int) (100f * newBrightnessPercentage);
                    showChangeBrightness(newBrightnessProgress);
                }
                if (mNeedChangeVolume) {
                    deltaY = -deltaY;
                    int maxVolume = mNiceVideoPlayer.maxVloume();
